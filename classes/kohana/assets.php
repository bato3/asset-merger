...
abstract class Kohana_Assets {
    ...
    /**
     * @var  array  unmerged assets
     */
    protected $_unmerged = array();
    ...
    public function __construct($name = 'all')
    {
        ...
        $this->_process = in_array(Kohana::$environment, (array) Kohana::$config->load('asset-merger.process'));
    ...
    protected function add($class, $type, $file, array $options = array())
    {
        ...
        //Check if merge is set in $options, and if is, check if it's false
        elseif (array_key_exists('merge', $options) AND !Arr::get($options, 'merge'))
        {
            // Add to unmerged
            $this->_unmerged[] = new $class($type, $file, $options);
        }
        else
        {
